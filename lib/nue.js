...
function map(value, result, i, results, isLast, end) {
  results[i] = result;
  return true;
}

function filter(value, result, i, results, isLast, end) {
  if (result) {
    results.push(value);
  }
  return true;
...
function every(value, result, i, results, isLast, end) {
  if (!result) {
    end(false);
    return false;
  } else if (isLast) {
...
function some(value, result, i, results, isLast, end) {
  if (result) {
    end(true);
    return false;
  } else if (isLast) {
...
function runEach(batchSize, worker, callerArgs, callerContext, strategy) {
  ...
  function EachContext(value, values, index, batchSize, data) {
    ContextBase.call(this);
    this.value = value;
    this.values = values;
    this.index = index;
  ...
  EachContext.prototype.next = function (result) {
    var next = strategy(this.value, result, index, results, this.isLast, endEach);
    index++;
  ...
  function runWorker(value, values) {
    worker.call(new EachContext(value, values, index, batchSize, callerContext.data), value);
  }
