...
LastStepContext.prototype.invoke = function invoke() {
  try {
    ...
    // thrown in a last function
    throw new LastStepError(e);
  }
};

LastStepContext.prototype._exit = function _exit() {
  if (this.flow.err) {
    // an error has not been handled in a flow
    var err = new UnhandleError(this.flow.err, 'An error has not been handled. To avoid this behavior, add the statement "this.err = null;" ' +
          'before exiting the last function.');
    err.stack += '\nCaused by: ' + this.flow.err.stack;
    throw err;
  }
...
util.inherits(NueError, Error);

function UnhandleError() {
  NueError.apply(this, arguments);
}
util.inherits(UnhandleError, NueError);

function LastStepError(e) {
  NueError.call(this);
  this.message = e.message;
  this.name = e.name;
  this.stack = e.stack;
}
util.inherits(LastStepError, NueError);