...
function runFlow(flowName, steps, callerArgs, callerContext) {
  ...
  steps = chainSteps(steps, callerContext, flow);
...
function chainSteps(steps, callerContext, flow) {
  ...
  steps[steps.length - 1].events.once('done', function exitFlow(err, args) {
