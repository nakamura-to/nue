...
Async.prototype.makeCallback = function makeCallback(mapping) {
  ...
  return function asyncCallback(err) {
    ...
    if (!self.isCanceled) {
      if (isErrorHandleRequire(mapping) && err) {
        err = err instanceof NueAsyncError ? err : new NueAsyncError(err, self.flowName, self.stepName, self.stepIndex, index);
