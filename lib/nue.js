...
Nue.prototype.parallel = function (tasks) {
  var batchSize = this.batchSize;
  var fn = function () {
    var args = Array.prototype.slice.apply(arguments);
        ...
        (function execute(task, tasks) {
          var context = {
            ...
            next: function () {
              ...
              data = context.data;
              if (args[0]) {
                context.end.apply(null, args);
              } else {
                result = args.slice(1);
                if (result.length === 0) {
                  results[valueIndex] = undefined;
                } else if (result.length === 1) {
                  results[valueIndex] = result[0];
                } else {
                  results[valueIndex] = result;
                }
                valueIndex++;
                if (tasks.length) {
          ...
          worker.call(context, null, task);
        }(tasks.shift(), tasks));
      }());
    }
  };
  context.next.apply(null, callerArgs);
}
