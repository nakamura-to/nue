...
function runParallel(flowName, functions, callerArgs, callerContext) {
  callerContext.each(1)(functions, function (fn, group) {
...
ContextBase.SIGNAL_DECR_LOCK_COUNT = {};
    ...
    function callback(err) {
      ...
      if (!self._isAsyncCanceled) {
        if (err) {
          ...
          if (args[0] === ContextBase.SIGNAL_DECR_LOCK_COUNT) {
            self._asyncLockCount--;
          } else {
            self._results[asyncIndex] = args.concat(Array.prototype.slice.call(arguments, 1));
          }
          if (self._asyncCallCount === 0 && self._asyncLockCount === 0) {
...
ContextBase.prototype._asyncGroup = function _asyncGroup() {
  ...
  var asyncLockCount = 1;
  var asyncIndex = 0;
  var results = [];
  return function asyncGroup() {
    asyncCallCount++;
    var args = Array.prototype.slice.call(arguments);
    return (function makeCallback(args, asyncIndex) {
      function callback(err) {
        ...
        if (err) {
          ...
          if (asyncCallCount === 0 && asyncLockCount === 0) {
...
ContextBase.prototype.each = function each() {
  var self = this;
  ...
  function consume(concurrency, array, worker, group, index) {
    ...
    if (index === len) {
      process.nextTick(group(ContextBase.SIGNAL_DECR_LOCK_COUNT));
      process.nextTick(self.async(ContextBase.SIGNAL_DECR_LOCK_COUNT));
    } else {
