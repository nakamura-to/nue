    ...
    function callback(err) {
      ...
      if (!self._isAsyncCanceled) {
        if (err) {
          ...
          if (args[0] === ContextBase.UNLOCK_MESSAGE) {
            self._lock = false;
          } else {
...
ContextBase.prototype.forEach = function forEach() {
  ...
  if (arguments.length === 1 && typeof arguments[0] === 'number') {
    ...
    return function () {
      lockAndConsume(concurrency, arguments[0], arguments[1]);
    };
  } else {
    lockAndConsume(ContextBase.DEFAULT_CONCURRENCY, arguments[0], arguments[1]);
  }

  function lockAndConsume(concurrency, array, worker) {
  ...
  function consume(concurrency, originalArray, consumedArray, worker, index) {
    ...
    for (i = 0; i < values.length; i++) {
      worker.call(self, values[i], i + index, originalArray);
    }
    if (consumedArray.length === 0) {
      process.nextTick(self.async(ContextBase.UNLOCK_MESSAGE));
    } else {
      index += len;
      process.nextTick(function consumeNext() {
        consume(concurrency, originalArray, consumedArray, worker, index);
      });
    }
  }
};
