...
function runFlow(batchSize, tasks, caller, callerArgs, data) {
  ...
  function startStep(step, flow) {
    flow.callCount++;
    step.apply(new StepContext(step, flow, endStep), flow.args);
  }
  ...
  function makeSteps(tasks) {
    ...
    steps.forEach(function (task, i, tasks) {
      var next = tasks[i + 1];
      if (i < tasks.length - 1) {
        if (task.__nue__) {
...
function runEach(batchSize, worker, callerArgs, callerContext) {
  ...
  function WorkerContext(values, index, data) {
    this.values = values;
    this.index = index;
    this.data = data;
  ...
  WorkerContext.prototype.next = function () {
    var args = Array.prototype.slice.apply(arguments);
    switch (args.length) {
  ...
  function runWorker(value, values) {
    worker.call(new WorkerContext(values, index, data), value);
  }
...
function runParallel(batchSize, tasks, callerArgs, callerContext) {
  ...
  function TaskContext(data, index) {
    this.data = data;
    this.index = index;
    this.next = this.next.bind(this);
    this.end = this.end.bind(this);
  ...
  TaskContext.prototype.next = function (result) {
    data = this.data;
    results[this.index] = result;
    taskCount--;
  };

  TaskContext.prototype.end = function () {
    callerContext.err = arguments[0];
    data = this.data;
    isCanceled = true;
  ...
  TaskContext.prototype.callback = function (err, result) {
    if (arguments[0]) {
      this.end.apply(this, arguments);
    } else {
      this.next.apply(this, Array.prototype.slice.call(arguments, 1));
  ...
  function processTasks (data) {
    ...
    if (taskCount === 0) {
      ...
      for (i = 0; i < len; i++) {
        ...
        task = tasks[i];
        task.value.call(new TaskContext(data, task.index), parallelArgs[i]);
      }
      process.nextTick(function () {
        processTasks(data);
      });
