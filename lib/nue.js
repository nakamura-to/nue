...
function parallel() {
  ...
  function prepareParallel(flowName, functions) {
    function startParallel() {
      assert.ok(this instanceof StepContext, 'A `parallel` must be inside a flow or another parallel.');
      var self = this;
      var args = Array.prototype.slice.call(arguments);
...
Async.prototype.makeCallback = function makeCallback(mapping, caller) {
  assert.ok(typeof mapping === 'object' && mapping !== null, 'An argument `mapping` must be an object');
  this.pending++;
...
StepContext.prototype.asyncEach = function asyncEach() {
  ...
  if (arguments.length === 1 && typeof arguments[0] === 'number') {
    ...
    validateAndConsume(StepContext.DEFAULT_CONCURRENCY, arguments[0], arguments[1]);
  }
