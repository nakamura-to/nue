...
function AsyncError(cause, stepName, stepIndex, asyncIndex) {
  this.cause = cause;
    ...
    return function (err) {
      ...
      if (!self.isAsyncCanceled) {
        if (err) {
          ...
          self.endWithErr.call(self, new AsyncError(err, self.step.stepName, self.step.stepIndex, asyncIndex));
        } else {
