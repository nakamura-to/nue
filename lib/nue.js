...
ContextBase.prototype.async = function async() {
  ...
  var index = mapping === ContextBase.SIGNAL_UNLOCK ? -1 : this._asyncIndex++;
  var self = this;  
  return (function makeCallback(mapping, asyncIndex) {
    ...
    function asyncCallback(err) {
      ...
      if (!self._isAsyncCanceled) {
        if (err) {
          ...
          if (mapping === ContextBase.SIGNAL_UNLOCK) {
            self._lock = false;
          } else {
            self._results[asyncIndex] = mapping ? mapArguments(mapping, arguments) : Array.prototype.slice.call(arguments, 1);
          }
          if (self._asyncCallCount === 0 && !self._lock) {
...
ContextBase.prototype.asyncEach = function asyncEach() {
  ...
  function waitAndConsume(concurrency, array, worker) {
    ...
    var group = makeAsyncGroup(self.async());
  ...
  function makeAsyncGroup(callback) {
    ...
    function asyncGroup() {
      var mapping = arguments[0];
      var index = mapping === ContextBase.SIGNAL_UNLOCK ? -1 : asyncIndex++;
      asyncCallCount++;
      return (function makeCallback(mapping, asyncIndex) {
        ...
        function asyncGroupCallback(err) {
          ...
          if (err) {
            ...
            if (mapping === ContextBase.SIGNAL_UNLOCK) {
              ...
              results[asyncIndex] = mapping ? mapArguments(mapping, arguments) : Array.prototype.slice.call(arguments, 1);
            }
            if (asyncCallCount === 0 && !lock) {
              callback(null, normalizeArray(results));
            }
          }
        }
      }(mapping, index));
...
ContextBase.prototype.await = function await() {
  this.async(ContextBase.SIGNAL_UNLOCK)();
};
