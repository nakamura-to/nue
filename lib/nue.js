 ...
 * @param worker
 */
function SerialQueue(batchSize, worker) {
  ...
  function executeBatch (args) {
    ...
    (function execute(value, values, args) {
      var context = {
        ...
        isLast: (function (isAddingCompleted, queueIndex, pushCount) {
          return function () {
            return isAddingCompleted && queueIndex === pushCount - 1;
          }
        }(self.isAddingCompleted, self.queueIndex, self.pushCount)),
        next: function () {
          var args = Array.prototype.slice.apply(arguments);
          context.next = noOp;
          self.queueIndex++;
 ...
 * @param tasks
 */
function Serial(batchSize, tasks) {
  var worker = function(task) {
    var args = Array.prototype.slice.call(arguments, 1);
    task.apply(this, args);
  };
  var queue = new SerialQueue(batchSize, worker);
...
Nue.prototype.serial = function () {
  ...
  return function () {
    ...
    return new Serial(self.batchSize, tasks);
  };
};
